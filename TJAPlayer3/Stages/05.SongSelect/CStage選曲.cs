	...
	internal class CStage選曲 : CStage
	{
		...
		public override int On進行描画()
		{
			if (!base.b活性化してない)
			{
				...
				double 回転値 = 180.0;
				int u;
				u = (int)(Math.Sin(this.act曲リスト.tenkai.n現在の値 * Math.PI / (this.act曲リスト.tenkai.n終了値 * 180.0 / 回転値)) * 20);
				double a;
				a = Math.Sin(tenkai2.n現在の値 * Math.PI / tenkai2.n終了値) * 0.3;


				if (this.act難易度選択画面.bIsDifficltSelect)
				{
					TJAPlayer3.Tx.SongSelect_Mark.Opacity = ((toumei * 0xff) / 100);
					TJAPlayer3.Tx.SongSelect_Mark.vc拡大縮小倍率.Y = (float)(a + 1f);
					TJAPlayer3.Tx.SongSelect_Mark.t2D拡大率考慮下中心基準描画(TJAPlayer3.app.Device, 140, TJAPlayer3.Skin.SongSelect_Overall_Y + 316 - (float)a, new Rectangle(360 * this.act難易度選択画面.n現在の選択行, 0, 360, 314));

					#region background
					TJAPlayer3.Tx.SongSelect_nanido.Opacity = ((toumei * 0xff) / 100);
					TJAPlayer3.Tx.SongSelect_nanido2.Opacity = ((toumei * 0xff) / 100);
					...
					{
						if (TJAPlayer3.stage選曲.n現在選択中の曲の難易度 != 4 && this.act難易度選択画面.n現在の選択行 >= 0)
						{
							TJAPlayer3.Tx.SongSelect_Score_Select?.t2D下中央基準描画(TJAPlayer3.app.Device, 497 + (this.act難易度選択画面.n現在の選択行 * 144), TJAPlayer3.Skin.SongSelect_Overall_Y + 406, new Rectangle(0, 130, 260, 270));

						}
						else if (this.act難易度選択画面.n現在の選択行 >= 0)
						{
						...
						{
							TJAPlayer3.Tx.SongSelect_Score_Select?.t2D下中央基準描画(TJAPlayer3.app.Device, 469 + (this.act難易度選択画面.n現在の選択行 * 88), TJAPlayer3.Skin.SongSelect_Overall_Y + 232, new Rectangle(0, 400, 260, 100));
						}

					}
					TJAPlayer3.Tx.SongSelect_nanido2.t2D描画(TJAPlayer3.app.Device, 0, 0);

					#endregion
					#region nanido
					if (TJAPlayer3.Tx.SongSelect_nanido != null)
					{
						// 難易度がTower、Danではない
						if (TJAPlayer3.stage選曲.n現在選択中の曲の難易度 != (int)Difficulty.Tower && TJAPlayer3.stage選曲.n現在選択中の曲の難易度 != (int)Difficulty.Dan)
						{
							for (int i = 0; i < (int)Difficulty.Edit + 1; i++)
							{
								...
								{
									// レベルが0以上
									TJAPlayer3.Tx.SongSelect_nanido.color4 = new Color4(1f, 1f, 1f);
									if (i != 4)
									{
										TJAPlayer3.Tx.SongSelect_nanido.t2D下中央基準描画(TJAPlayer3.app.Device, 497 + (i * 144), TJAPlayer3.Skin.SongSelect_Overall_Y + 386, new Rectangle(140 * i, 0, 140, 240));
										if (this.act難易度選択画面.n現在の選択行 >= 0)
											TJAPlayer3.Tx.SongSelect_Score_Select?.t2D下中央基準描画(TJAPlayer3.app.Device, 497 + (this.act難易度選択画面.n現在の選択行 * 144), TJAPlayer3.Skin.SongSelect_Overall_Y + 176 + u, new Rectangle(0, 0, 260, 115));
										if (this.act難易度選択画面.n現在の選択行 < 0)
											TJAPlayer3.Tx.SongSelect_Score_Select?.t2D下中央基準描画(TJAPlayer3.app.Device, 469 + (this.act難易度選択画面.n現在の選択行 * 88), TJAPlayer3.Skin.SongSelect_Overall_Y + 176 + u, new Rectangle(0, 0, 260, 115));

									}
									if (i == 4 && this.act難易度選択画面.n現在の選択行 == 4)
									{
										// エディット
										TJAPlayer3.Tx.SongSelect_nanido.t2D下中央基準描画(TJAPlayer3.app.Device, 497 + (3 * 144), TJAPlayer3.Skin.SongSelect_Overall_Y + 386, new Rectangle(140 * i, 0, 140, 240));
										TJAPlayer3.Tx.SongSelect_Score_Select?.t2D下中央基準描画(TJAPlayer3.app.Device, 497 + (4 * 144), TJAPlayer3.Skin.SongSelect_Overall_Y + 176 + u, new Rectangle(0, 0, 260, 115));
									}
								...
								{
									// レベルが0未満 = 譜面がないとみなす
									TJAPlayer3.Tx.SongSelect_nanido.color4 = new Color4(0.5f, 0.5f, 0.5f);
									if (this.act難易度選択画面.n現在の選択行 == 4)
									{
										// エディット
										TJAPlayer3.Tx.SongSelect_nanido.t2D下中央基準描画(TJAPlayer3.app.Device, 497 + (3 * 144), TJAPlayer3.Skin.SongSelect_Overall_Y + 386, new Rectangle(140 * i, 0, 140, 240));
									}
						...
						else
						{
							if (TJAPlayer3.stage選曲.r現在選択中のスコア.譜面情報.nレベル[TJAPlayer3.stage選曲.n現在選択中の曲の難易度] >= 0)
							{
								// 譜面がありますね
								TJAPlayer3.Tx.SongSelect_nanido.color4 = new Color4(1f, 1f, 1f);
								TJAPlayer3.Tx.SongSelect_nanido.t2D下中央基準描画(TJAPlayer3.app.Device, 497, TJAPlayer3.Skin.SongSelect_Overall_Y + 386, new Rectangle(140 * 4, 240, 140, 240));
							}
							else
							{
								// ないですね
				...
				if (base.eフェーズID == CStage.Eフェーズ.共通_通常状態)
				{
										...
										case C曲リストノード.Eノード種別.SCORE:
											{
												...
												this.act難易度選択画面.bIsDifficltSelect = true;
												this.act難易度選択画面.n現在の選択行 = 0;
												tenkai2 = new CCounter(0, 353, 1, TJAPlayer3.Timer);
					...
					else if (!this.actSortSongs.bIsActivePopupMenu && !this.actQuickConfig.bIsActivePopupMenu && TJAPlayer3.stage選曲.n現在選択中の曲の難易度 != (int)Difficulty.Dan)
					{
						...
						if (TJAPlayer3.Pad.b押された(E楽器パート.DRUMS, Eパッド.RBlue))
						{
							...
							this.act曲リスト.tenkai = new CCounter(0, 163, 1, TJAPlayer3.Timer);
							TJAPlayer3.Skin.sound変更音.t再生する();
						}
